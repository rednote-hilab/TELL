...
class Agent(ABC):
    ...
    def _on_pre_loop(self) -> None:
        """Hook called before the main game loop. Override in subclasses."""
        pass
    ...
    def main(self) -> None:
        ...
        try:
            ...
            if self.is_done(self.frames, self.frames[-1]):
                ...
                self._end_reason = "max_actions"
            else:
                self._end_reason = "loop_exit"
        except KeyboardInterrupt:
            self._end_reason = "keyboard_interrupt"
            raise
        except Exception as exc:
            msg = str(exc)
            if "worker_failed" in msg:
                self._end_reason = "worker_failed"
            elif "worker_stopped" in msg:
                self._end_reason = "worker_stopped"
            else:
                self._end_reason = "exception"
            raise
        finally:
            if self._end_reason == "keyboard_interrupt":
                logger.info(
                    "[run_interrupted] game_id=%s reason=%s state=%s levels_completed=%s actions=%s max_actions=%s",
                    self.game_id,
                    self._end_reason,
                    self.frames[-1].state,
                    self.frames[-1].levels_completed,
                    self.action_counter,
                    self.MAX_ACTIONS,
                )
            else:
                logger.info(
                    "[run_end] game_id=%s reason=%s state=%s levels_completed=%s actions=%s max_actions=%s",
                    self.game_id,
                    self._end_reason,
                    self.frames[-1].state,
                    self.frames[-1].levels_completed,
    ...
    def cleanup(self, scorecard: Optional[EnvironmentScorecard] = None) -> None:
        ...
        if self._cleanup:
            ...
            if self.action_counter >= self.MAX_ACTIONS:
                logger.info(
                    f"Exiting: reason={self._end_reason}, reached MAX_ACTIONS of {self.MAX_ACTIONS}, took {self.seconds} seconds ({self.fps} average fps)"
                )
            else:
                logger.info(
                    f"Finishing: reason={self._end_reason}, agent took {self.action_counter} actions, took {self.seconds} seconds ({self.fps} average fps)"
                )
